...
init = {
    ...
    weights: 'yolov8n.pt',
    classes: 0.0,  # 要检测的标签的名字 别改这里改tool.py 159 行
    ...
    while True:
        try:
            ...
            if data[show] and image is not None:
                # 记录耗时
                cv2.putText(image, f'{Timer.cost(t3 - t1)}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255),
                            1)
                cv2.putText(image, f'{Timer.cost(t2 - t1)}', (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255),
                            1)
                cv2.putText(image, f'{Timer.cost(t3 - t2)}', (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255),
                            1)
                # 瞄点划线
                if target:
                    index, clazz, conf, sc, gc, sr, gr = target
                    ...
                    box = (gr[0], gr[1] - 100, gr[2] + gr[0], gr[3] + gr[1])
