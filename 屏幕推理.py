...
def capture_screen_around_center():
    screenshot = ImageGrab.grab((0, 0, 1920, 1080))
    img = np.array(screenshot)  # Convert the screenshot to a numpy array
    img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)  # Convert to BGR color format
    cv2.imwrite('detect.png', img)  # Save the image as 'detect.png'
    return img
...
while True:
    ...
    image = capture_screen_around_center()
    image = cv2.resize(image, (400, 400))  # Resize the image to 400x400
    t2 = time.perf_counter_ns()
    final_aims = detect(image)
    t3 = time.perf_counter_ns()
    lst = []
    for cls, name, conf, boxes_xywh in final_aims:
        box = (boxes_xywh[0], boxes_xywh[1], boxes_xywh[2] + boxes_xywh[0], boxes_xywh[3] + boxes_xywh[1])
        cv2.rectangle(image, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (221, 0, 27), 2)
        text = f"{name}: {conf:.2f}"  # Format the text with name and confidence
        cv2.putText(image, text, (int(box[0]), int(box[1]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
